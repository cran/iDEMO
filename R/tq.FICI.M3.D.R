tq.FICI.M3.D <-
function( P3, alpha, W, Eta.M3, sEta.M3, sB.M3, Cov.Mat.M3, n, range.t, ... ){

	tp.FICI.M3.cal <- function(p3){
		...
		var.tp.FI.M3 <- ( t(diff.cdf.M3) %*% Cov.Mat.M3 %*% diff.cdf.M3 ) / ( f_T.M3(tp)^2 ) / n

		CI.lower.tp.M3 <- tp - qnorm( 1 - alpha / 2 ) * sqrt(var.tp.FI.M3)

		CI.upper.tp.M3 <- tp + qnorm( 1 - alpha / 2 ) * sqrt(var.tp.FI.M3)


		var.tp.FI.ln.M3 <- ( t(diff.cdf.M3) %*% Cov.Mat.M3 %*% diff.cdf.M3 ) / ( tp * f_T.M3(tp) )^2 / n

		CI.ln.lower.tp.M3 <- exp( log(tp) - qnorm( 1 - alpha / 2 ) * sqrt(var.tp.FI.ln.M3) )

		CI.ln.upper.tp.M3 <- exp( log(tp) + qnorm( 1 - alpha / 2 ) * sqrt(var.tp.FI.ln.M3) )

		c( tp, CI.lower.tp.M3, CI.upper.tp.M3, CI.ln.lower.tp.M3, CI.ln.upper.tp.M3, diff.cdf.M3 )

	}

	matrix( apply( as.matrix(P3), 1, tp.FICI.M3.cal ), length(P3), 8, byrow=TRUE )
