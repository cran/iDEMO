tq.OICI.M2.D <-
function( P2, alpha, W, Eta.M2, sB.M2, Cov.Mat.M2, range.t, ... ){

	tp.OICI.M2.cal <- function(p2){
		...
		var.tp.OI.M2 <- ( t(diff.cdf.M2) %*% Cov.Mat.M2 %*% diff.cdf.M2 ) / ( f_T.M2(tp)^2 )

		CI.loWer.tp.M2 <- tp - qnorm( 1 - alpha / 2 ) * sqrt(var.tp.OI.M2)

		CI.upper.tp.M2 <- tp + qnorm( 1 - alpha / 2 ) * sqrt(var.tp.OI.M2)




		var.tp.OI.ln.M2 <- ( t(diff.cdf.M2) %*% Cov.Mat.M2 %*% diff.cdf.M2 ) / ( tp * f_T.M2(tp) )^2

		CI.ln.loWer.tp.M2 <- exp( log(tp) - qnorm( 1 - alpha / 2 )*sqrt(var.tp.OI.ln.M2) )

		CI.ln.upper.tp.M2 <- exp( log(tp) + qnorm( 1 - alpha / 2 )*sqrt(var.tp.OI.ln.M2) )

		c( tp, CI.loWer.tp.M2, CI.upper.tp.M2, CI.ln.loWer.tp.M2, CI.ln.upper.tp.M2 )

	}
	matrix( apply( as.matrix(P2), 1, tp.OICI.M2.cal ), length(P2), 5, byrow=TRUE )
