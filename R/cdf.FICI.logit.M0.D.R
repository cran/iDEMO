cdf.FICI.logit.M0.D <-
function( Par.M0, t, Cov.Matrix.FI.M0, alpha, W, n, ... ) {
	...
	cdf.FICI.logit.M0.cal <- function(t){
		...
		var.cdf.FI.M0 <-   c( t(  diff.cdf.M0 )  %*% Cov.Matrix.FI.M0 %*% diff.cdf.M0 ) / n 

		W.FI.M0 <-  exp(   qnorm( 1 - alpha / 2 ) *  sqrt(  var.cdf.FI.M0 ) /  ( cdf.M0 *  ( 1 - cdf.M0 ) ) ) 
  
		FICI.lower.cdf.M0 <-  cdf.M0 /  ( cdf.M0 +  ( 1 - cdf.M0 ) *  W.FI.M0  ) 

		FICI.upper.cdf.M0 <-  cdf.M0 /  ( cdf.M0 +  ( 1 - cdf.M0 ) /  W.FI.M0  ) 


		return( c( cdf.M0, FICI.lower.cdf.M0, FICI.upper.cdf.M0 ) )

	}


	M <- matrix( apply( as.matrix(t), 1, cdf.FICI.logit.M0.cal ), 3, length(t), byrow=FALSE )

	cdf.M0.out <- M[1,]

	FICI.lower.cdf.M0.out <- M[2,]

	FICI.upper.cdf.M0.out <- M[3,]

	list( cdf.M0 = cdf.M0.out, lcl.cdf.M0 = FICI.lower.cdf.M0.out, ucl.cdf.M0 = FICI.upper.cdf.M0.out )
