tq.OICI.M0.D <-
function( P0, alpha, W, Eta.M0, sEta.M0, sB.M0, Cov.Mat.M0, range.t, ... ){

	tp.OICI.M0.cal <- function(p0){
		...
		#Ft.M0 <- expression( pnorm( ( Eta.M0 * tp - W ) / sqrt( sB.M0^2 * tp + sEta.M0^2 * tp^2 ) ) + 

			#exp( 2 * Eta.M0 * W / sB.M0^2 ) * pnorm( -( 2 * sEta.M0^2 * W * tp + sB.M0^2 * ( Eta.M0 * tp + W ) ) /

			#( sB.M0^2 * sqrt( sB.M0^2 * tp + sEta.M0^2 * tp^2 ) ) ) * exp( sEta.M0^2 * W^2 / ( 4 * sB.M0^4 ) ) *

	            #exp( sEta.M0^2 * W^2 / ( 4 * sB.M0^4 ) ) * exp( sEta.M0^2 * W^2 / ( 4 * sB.M0^4 ) ) * 

	            #exp( sEta.M0^2 * W^2 / ( 4 * sB.M0^4 ) ) * exp( sEta.M0^2 * W^2 / ( 4 * sB.M0^4 ) ) * 

	            #exp( sEta.M0^2 * W^2 / ( 4 * sB.M0^4 ) ) * exp( sEta.M0^2 * W^2 / ( 4 * sB.M0^4 ) ) * 

	            #exp( sEta.M0^2 * W^2 / ( 4 * sB.M0^4 ) ) )
		...
		var.tp.OI.M0 <- ( t(diff.cdf.M0) %*% Cov.Mat.M0 %*% diff.cdf.M0 ) / ( f_T.M0(tp)^2 )

		CI.lower.tp.M0 <- tp - qnorm( 1 - alpha / 2 ) * sqrt(var.tp.OI.M0)

		CI.upper.tp.M0 <- tp + qnorm( 1 - alpha / 2 ) * sqrt(var.tp.OI.M0)




		var.tp.OI.ln.M0 <- ( t(diff.cdf.M0) %*% Cov.Mat.M0 %*% diff.cdf.M0 ) / ( tp * f_T.M0(tp) )^2

		CI.ln.lower.tp.M0 <- exp( log(tp) - qnorm( 1 - alpha / 2 ) * sqrt(var.tp.OI.ln.M0) )

		CI.ln.upper.tp.M0 <- exp( log(tp) + qnorm( 1 - alpha / 2 ) * sqrt(var.tp.OI.ln.M0) )

		c( tp, CI.lower.tp.M0, CI.upper.tp.M0, CI.ln.lower.tp.M0, CI.ln.upper.tp.M0, diff.cdf.M0 )

	}

	matrix( apply( as.matrix(P0), 1, tp.OICI.M0.cal ), length(P0), 9, byrow=TRUE )
